...
def gethousesize_byfulldes(soup):
    size_ft_by_des = 0
    house_descriptionhtml = soup.find("div", class_="dp-description__text")
    house_description = house_descriptionhtml.get_text("\n", strip=True)

    # Get house size through square footage
    square_footage_text = re.findall(r"square footage: [0-9]+[.]?[0-9]*", house_description, flags=re.IGNORECASE)
    ...
    if key_feature:
        ...
        for i in key_feature:
            ...
            if i.startswith("total"):
                size_by_key_feature = re.findall(r"[0-9]+[.][0-9]+sq.m", i)
                if size_by_key_feature:
                    size_ft_by_des = float(size_by_key_feature[0].replace("sq.m", "")) * 10.7639
                    return size_ft_by_des
            if re.findall(r"internalarea", i):
                size_by_key_feature = re.findall(r'[0-9]+[.]?[0-9]sq[.]?ft', i)
                if size_by_key_feature:
                    size_by_key_feature = re.findall(r"[0-9]+[.]?[0-9]", size_by_key_feature[0])
                    size_ft_by_des = float(size_by_key_feature[0])
                    return size_ft_by_des

    house_description = house_description.replace('”', '"').replace('"', '$').replace("’", "'")
    house_text = house_description.split("\n")
    ...
    for k in sq_inch:
        size_ft_by_des = size_ft_by_des + k
...
def gethousesize_byfloorplan(soup):
    ...
    if floorplan:
        ...
        if floorplan:
            ...
            floor_planfolder_path = "./floorplan/"
            if not os.path.exists(floor_planfolder_path):
                os.makedirs(floor_planfolder_path)

            floor_plan_link = requests.get(floorplan_url)
            img_name = floor_planfolder_path + "floorplan.jpg"
            with open(img_name, "wb") as file:
                file.write(floor_plan_link.content)
                file.flush()
            file.close()
            ...
            for i in range(0, len(text)):
                if text[i].lower().startswith("total"):
                    sizeby_photo = re.findall(r"[0-9]+[.]?[0-9]*", text[i])
                    if not sizeby_photo:
                        sizeby_photo = re.findall(r"[0-9]+", text[i])
                    break

            for i in range(0, len(sizeby_photo)):
                sizeby_photo[i] = float(sizeby_photo[i])
    else:
        sizeby_photo = []
...
def webscraping(url):
    ...
    if page.status_code == 200:
        ...
        sizeby_photo = gethousesize_byfloorplan(soup)
        size_photo = 0
        if sizeby_photo:
            if len(sizeby_photo) >= 2:
                size_photo = max(sizeby_photo)

            if len(sizeby_photo) == 1:
                size_photo = float(sizeby_photo[0])
